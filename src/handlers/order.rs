...
pub async fn mark_order_shipped(pool: web::Data<MySqlPool>, order_id: web::Path<i64>) -> impl Responder {
...
pub async fn mark_delivered(pool: web::Data<MySqlPool>, order_id: web::Path<i64>) -> impl Responder {
    let order_id = order_id.into_inner();
...
pub async fn get_all_orders(pool: web::Data<MySqlPool>, query: web::Query<Pagination>) -> impl Responder {
...
pub async fn get_order_by_id(pool: web::Data<MySqlPool>, order_id: web::Path<i64>) -> impl Responder {
    let order_id = order_id.into_inner();
...
pub async fn get_user_orders(pool: web::Data<MySqlPool>, email: web::Path<String>, query: web::Query<Pagination>) -> impl Responder {
    let email = email.into_inner();
    let page = query.page.max(1);
    let per_page = query.per_page.clamp(1, 100);
    let offset = (page - 1) * per_page;

    //count total orders for uder
    ...
    let total = match total_result {
        Ok(r) => r.count,
        Err(e) => {
            eprintln!("Count user orders error: {}", e);
            return HttpResponse::InternalServerError().body("Failed to count orders");
        }
    };

    //fetch paginaed orders
    let orders = sqlx::query_as::<_, Order>(
        "SELECT id, cart_id, status, email, address, delivery_date, created_at, updated_at 
         FROM orders WHERE email = ? ORDER BY created_at DESC LIMIT ? OFFSET ?"
    )
    .bind(&email)
    .bind(per_page)
    .bind(offset)
    ...
    match orders {
        ...
        Err(e) => {
            eprintln!("Fetch user orders error: {}", e);
            HttpResponse::InternalServerError().body("Failed to fetch orders")
        }
    }
}
...
/*
    orders will only be cancelled after confirmation, i.e the order hsant been shipped
    then a refund  policy will be created to handle funds refund or whether convert to grof 
    points
 */
pub async fn cancel_order(pool: web::Data<MySqlPool>, order_id: web::Path<i64>) -> impl Responder {
    let order_id = order_id.into_inner();

    let result = sqlx::query!(
        "UPDATE orders SET status = 'cancelled', updated_at = NOW() WHERE id = ? AND status = 'confirmed'",
        order_id
    )
    .execute(pool.get_ref())
    .await;
